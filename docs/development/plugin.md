...
> 下面示范的插件工程可以在 [插件范例](https://github.com/chuanwise/xiaoming-example) 处下载到。欢迎 `clone` 这个仓库对照研究。如果你的 `IDE` 是 `IDEA`，`clone` 下来的工程无法编译，请参考[开发前的准备](/development/)马上开始你的插件编写之旅吧！
...
```log
[2021-15-18 21:15:54] [pool-1-thread-4] [INFO] c.c.x.p.PluginManagerImpl : 正在加载插件：example-plugin
[2021-15-18 21:15:54] [pool-1-thread-4] [INFO] c.c.x.p.PluginManagerImpl : 成功加载插件：example-plugin
[2021-15-18 21:15:54] [main] [INFO]            c.c.x.p.PluginManagerImpl : 正在启动插件：example-plugin
[2021-15-18 21:15:54] [main] [INFO]                       example-plugin : 小明示范插件启动成功，芜湖！
[2021-15-18 21:15:54] [main] [INFO]            c.c.x.p.PluginManagerImpl : 成功启动插件：example-plugin
```
...
### 提取参数
一些指令需要提取消息中的参数，例如 `设置小明名称  {name}`，其中的 `{name}` 需要被提取出来。这个工作在交互方法内进行有些麻烦，需要讨论参数数组的长度等。小明会帮你做这些操作，只需要在过滤器内使用 `{parameter}` 定义变量，并在交互方法的参数中使用 **过滤器参数注解** `@FilterParameter(...)` 引用它们就可以了。
...
`@Permission`|所需权限|如果用户没有该权限，将警告 `小明不能帮你做这件事哦，因为你缺少权限：{permission}`。其中可以包含 `@Filter` 的变量，例如 `@Filter("禁言 {qq}")` 和 `@Permission("mute.{args.qq}")`
...
插件主类中调用 `getXiaomingBot().getFileLoader().loadOrSupply(...)` 从插件数据文件夹 `getDataFolder()` 下读取一个文件 `configuration.json`。如果读取失败，则调用其构造函数构造一个默认值。
...
public class ExamplePlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        final ExampleConfiguration exampleConfiguration = getXiaomingBot().getFileLoader().loadOrSupply(ExampleConfiguration.class, new File(getDataFolder(), "configuration.json"), ExampleConfiguration::new);
    }
}
```
