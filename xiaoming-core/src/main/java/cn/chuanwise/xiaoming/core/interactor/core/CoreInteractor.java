...
public class CoreInteractor extends InteractorImpl {
    ...
    public void onListScheduler(XiaomingUser user) {
        ...
        builder.append("执行中：").append(CollectionUtility.toString(runningTasks, ScheduableTask::getDescription, "\n")).append("\n")
                .append("计划中：").append(CollectionUtility.toString(plannedTasks, task -> {
            return task.getDescription() + "：" + (task.isTimeout() ? "已过期" : TimeUtility.after(task.getDelay()));
        }, "\n"));
    ...
    public void onStatus(XiaomingUser user) {
        ...
        user.sendMessage( "{xiaoming}启动于：" + TimeUtility.format(statistician.getBeginTime()) + "\n" +
                 ...
                 "上次文件保存时间：" + TimeUtility.format(lastSaveTime) + "\n" +
                 "距今：" + TimeUtility.toTimeLength(System.currentTimeMillis() - lastSaveTime) + "\n" +
