...
import cn.codethink.xiaoming.message.basic.ImageType;
...
public class Mirais
    ...
    public static VipFaceType toXiaoMing(VipFace.Kind kind) {
        Preconditions.objectNonNull(kind, "vip face kind");
        final VipFaceType type = VIP_FACES.getKey(kind);
        Preconditions.elementNonNull(type);
        return type;
    }
     ...
     * @return mirai vip 表情类型
     * @throws NullPointerException             vipFaceType 为 null
     * @throws java.util.NoSuchElementException 没有对应的 mirai vip 表情类型
     */
    public static VipFace.Kind toMirai(VipFaceType vipFaceType) {
        Preconditions.objectNonNull(vipFaceType, "vip face");
        
        return Maps.getOrFail(VIP_FACES, vipFaceType);
    }
    ...
    private static final BidiMap<ImageType, net.mamoe.mirai.message.data.ImageType> IMAGE_TYPES = new DualLinkedHashBidiMap<>();
    
    // initialize mapper
    static {
        IMAGE_TYPES.put(ImageType.PNG, net.mamoe.mirai.message.data.ImageType.PNG);
        IMAGE_TYPES.put(ImageType.BMP, net.mamoe.mirai.message.data.ImageType.BMP);
        IMAGE_TYPES.put(ImageType.JPG, net.mamoe.mirai.message.data.ImageType.JPG);
        IMAGE_TYPES.put(ImageType.GIF, net.mamoe.mirai.message.data.ImageType.GIF);
        IMAGE_TYPES.put(ImageType.WEBP, net.mamoe.mirai.message.data.ImageType.APNG);
    }
    
    /**
     * 将 mirai 图片类型转化为小明图片类型
     *
     * @param imageType mirai 图片类型
     * @return 小明图片类型，或 null
     * @throws NullPointerException imageType 为 null
     */
    public static ImageType toXiaoMing(net.mamoe.mirai.message.data.ImageType imageType) {
    ...
    /**
     * 将小明图片类型转化为 mirai 图片类型
     *
     * @param imageType mirai 图片类型
     * @return mirai 图片类型
     */
    public static net.mamoe.mirai.message.data.ImageType toMirai(ImageType imageType) {
    ...
    public static MusicSoftwareType toXiaoMing(MusicKind musicKind) {
        final MusicSoftwareType musicSoftwareType = MUSIC_SOFTWARE_TYPES.getKey(musicKind);
        Preconditions.elementNonNull(musicSoftwareType);
        return musicSoftwareType;
    }
    ...
    public static MusicKind toMirai(MusicSoftwareType musicSoftwareType) {
        return Maps.getOrFail(MUSIC_SOFTWARE_TYPES, musicSoftwareType);
    }
     ...
     */
    public static MessageChain toMirai(CompoundMessage message, Map<Property<?>, Object> properties) {
    ...
    public static MessageSourceType toXiaoMing(MessageSourceKind kind) {
        final MessageSourceType sourceType = MESSAGE_SOURCE_TYPES.getKey(kind);
        Preconditions.elementNonNull(sourceType);
        return sourceType;
    }
     ...
     * @param type 小明消息源类型
     * @return mirai 消息源类型
     * @throws NoSuchElementException 没有对应的 mirai 消息源类型
     */
    public static MessageSourceKind toMirai(MessageSourceType type) {
        return Maps.getOrFail(MESSAGE_SOURCE_TYPES, type);
    }
    ...
    public static int toMiraiFaceId(PrimitiveFace primitiveFace) {
     ...
     * @return Mirai 表情
     */
    public static Face toMirai(PrimitiveFace primitiveFace) {
        return new Face(toMiraiFaceId(primitiveFace));
    }
     ...
     */
    public static MessageSource toMirai(cn.codethink.xiaoming.message.metadata.MessageSource messageSource, Map<Property<?>, Object> properties) {
        Preconditions.objectNonNull(messageSource, "message source");
        Preconditions.objectNonNull(properties, "properties");
    
        // if it's mirai reference, use it
        if (messageSource instanceof AbstractMiraiOnlineMessageSource) {
            return ((AbstractMiraiOnlineMessageSource) messageSource).getMiraiMessageSource();
        }
        if (messageSource instanceof MiraiOfflineMessageSource) {
            return ((MiraiOfflineMessageSource) messageSource).getMiraiMessageSource();
        }
        
        // build an offline message source
        final MessageSourceBuilder messageSourceBuilder = new MessageSourceBuilder()
            .messages(toMirai(messageSource.getMessage().asCompoundMessage(), properties))
        ...
        return messageSourceBuilder
            .build(messageSource.getBotCode().asLong(), toMirai(messageSource.getMessageSourceType()));
    }
    ...
    public static cn.codethink.xiaoming.message.metadata.MessageSource toXiaoMing(MessageSource messageSource, Map<Property<?>, Object> properties) {
        ...
        if (messageSource instanceof OfflineMessageSource) {
            ...
            return new MiraiOfflineMessageSource(offlineMessageSource, properties);
        }
        if (messageSource instanceof OnlineMessageSource) {
            final MiraiBot bot = (MiraiBot) Property.BOT.getOrFail(properties);
            ...
            if (onlineMessageSource instanceof OnlineMessageSource.Incoming.FromFriend) {
                final OnlineMessageSource.Incoming.FromFriend miraiMessageSource = (OnlineMessageSource.Incoming.FromFriend) onlineMessageSource;
        
                final MiraiFriend friend = bot.getFriendOrFail(Code.ofLong(miraiMessageSource.getFromId()));
                return new MiraiFromFriendMessageSource(miraiMessageSource, friend, properties);
            }
    
            if (onlineMessageSource instanceof OnlineMessageSource.Incoming.FromGroup) {
                final OnlineMessageSource.Incoming.FromGroup miraiMessageSource = (OnlineMessageSource.Incoming.FromGroup) onlineMessageSource;
    
                final MiraiGroup miraiGroup = bot
                    .getGroupOrFail(Code.ofLong(miraiMessageSource.getGroup().getId()));

                final Member sender = miraiMessageSource.getSender();
                final GroupSender groupSender;
                if (sender instanceof NormalMember) {
                    groupSender = miraiGroup.getMemberOrFail(Code.ofLong(sender.getId()));
                } else if (sender instanceof AnonymousMember) {
                    final AnonymousMember anonymousMember = (AnonymousMember) sender;
                    groupSender = miraiGroup.getAnonymous().getAvailable(anonymousMember);
                } else {
                    throw new IllegalArgumentException("unknown group member: " + sender);
                }
        
                return new MiraiFromGroupMessageSource(miraiMessageSource, groupSender, properties);
            }
    
            if (onlineMessageSource instanceof OnlineMessageSource.Incoming.FromStranger) {
                final OnlineMessageSource.Incoming.FromStranger miraiMessageSource = (OnlineMessageSource.Incoming.FromStranger) onlineMessageSource;
        
                final MiraiStranger stranger = bot.getStrangerOrFail(Code.ofLong(miraiMessageSource.getFromId()));
                return new MiraiFromStrangerMessageSource(miraiMessageSource, stranger, properties);
            }
    
            if (onlineMessageSource instanceof OnlineMessageSource.Incoming.FromTemp) {
                final OnlineMessageSource.Incoming.FromTemp miraiMessageSource = (OnlineMessageSource.Incoming.FromTemp) onlineMessageSource;
        
                final MiraiMember member = bot
                    .getGroupOrFail(Code.ofLong(miraiMessageSource.getGroup().getId()))
                    .getMemberOrFail(Code.ofLong(miraiMessageSource.getSender().getId()));
        
                return new MiraiFromGroupMemberMessageSource(miraiMessageSource, member, properties);
            }
    
            if (onlineMessageSource instanceof OnlineMessageSource.Outgoing.ToFriend) {
                final OnlineMessageSource.Outgoing.ToFriend miraiMessageSource = (OnlineMessageSource.Outgoing.ToFriend) onlineMessageSource;
        
                final MiraiFriend friend = bot.getFriendOrFail(Code.ofLong(miraiMessageSource.getTargetId()));
                return new MiraiToFriendMessageSource(miraiMessageSource, friend, properties);
            }
    
            if (onlineMessageSource instanceof OnlineMessageSource.Outgoing.ToGroup) {
                final OnlineMessageSource.Outgoing.ToGroup miraiMessageSource = (OnlineMessageSource.Outgoing.ToGroup) onlineMessageSource;
        
                final MiraiGroup group = bot.getGroupOrFail(Code.ofLong(miraiMessageSource.getTargetId()));
                return new MiraiToGroupMessageSource(miraiMessageSource, group, properties);
            }
    
            if (onlineMessageSource instanceof OnlineMessageSource.Outgoing.ToTemp) {
                final OnlineMessageSource.Outgoing.ToTemp miraiMessageSource = (OnlineMessageSource.Outgoing.ToTemp) onlineMessageSource;
        
                final MiraiMember member = bot.getGroupOrFail(Code.ofLong(miraiMessageSource.getGroup().getId()))
                    .getMemberOrFail(Code.ofLong(miraiMessageSource.getTargetId()));
        
                return new MiraiToGroupMemberMessageSource(miraiMessageSource, member, properties);
            }
    
            if (onlineMessageSource instanceof OnlineMessageSource.Outgoing.ToStranger) {
                final OnlineMessageSource.Outgoing.ToStranger miraiMessageSource = (OnlineMessageSource.Outgoing.ToStranger) onlineMessageSource;
        
                final MiraiStranger stranger = bot.getStrangerOrFail(Code.ofLong(miraiMessageSource.getTargetId()));
                return new MiraiToStrangerMessageSource(miraiMessageSource, stranger, properties);
            }
