...
public class EventForwarder
    ...
    public void onGroupMessageEvent(GroupMessageEvent event) {
        ...
        if (member instanceof NormalMember) {
            ...
            sender = ((QqGroup) group).getAnonymous().getAvailable(anonymousMember);
        } else {
            throw new NoSuchElementException("unknown member class: " + member.getClass());
        }
    
        final CompoundMessage compoundMessage = Qqs.toXiaoMing(event.getMessage(), Collections.singletonMap(Property.CONTACT, group));
    ...
    public void onGroupTempMessageEvent(GroupTempMessageEvent event) {
        ...
        final CompoundMessage compoundMessage = Qqs.toXiaoMing(event.getMessage(), Collections.singletonMap(Property.CONTACT, member));
    ...
    public void onFriendMessageEvent(FriendMessageEvent event) {
        ...
        final CompoundMessage compoundMessage = Qqs.toXiaoMing(event.getMessage(), Collections.singletonMap(Property.CONTACT, friend));
    ...
    public void onGroupMessageRecallEvent(MessageRecallEvent.GroupRecall event) {
        ...
        final net.mamoe.mirai.contact.Member qqOperator = event.getOperator();
        if (Objects.isNull(qqOperator)) {
            operator = bot;
        } else {
            operator = group.getMemberOrFail(Code.ofLong(qqOperator.getId()));
        }
