 ...
 * <h1>原生表情</h1>
 *
 * <p>消息码：{@code [face:primitive:$code]}</p>
 *
 * <p>摘要：{@code [$name]}</p>
 *
 * @author Chuanwise
...
@Getter
public enum PrimitiveFace
    implements Face, AutoSummarizable, AutoSerializable {
    ...
    private static final Map<String, PrimitiveFace> NAME_MAPPER = new HashMap<>();
    
    private static final Map<Integer, PrimitiveFace> CODE_MAPPER = new HashMap<>();
    
    private static final Set<PrimitiveFace> INSTANCES;
    
    static {
        final PrimitiveFace[] values = values();
        INSTANCES = Collections.asUnmodifiableSet(values);
        
        for (PrimitiveFace value : values) {
            NAME_MAPPER.put(value.name, value);
            CODE_MAPPER.put(value.code, value);
        }
    }
    
    PrimitiveFace(String name, int code) {
     ...
     * @return 所有原生表情
     */
    public static Set<PrimitiveFace> getInstances() {
        return INSTANCES;
    }
     ...
     * @param code 原生表情码
     * @return 原生表情
     * @throws java.util.NoSuchElementException 找不到原生表情
     */
    public static PrimitiveFace of(int code) {
        return Maps.getOrFail(CODE_MAPPER, code);
    }
     ...
     * @throws java.util.NoSuchElementException 找不到原生表情
     */
    public static PrimitiveFace of(String name) {
        return Maps.getOrFail(NAME_MAPPER, name);
    }
