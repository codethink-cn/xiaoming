...
import cn.chuanwise.xiaoming.api.account.record.GroupCommandRecord;
...
public interface Interactor extends PluginObject {
    ...
    default boolean interact(XiaomingUser user, Message message) throws Exception {
        ...
        for (InteractorMethodDetail detail : getMethodDetails()) {
            ...
            for (FilterMatcher filterMatcher : detail.getFilterMatchers()) {
            ...
            // 判断是否为同意使用条例
            final CommandFormat commandFormat = detail.getCommandFormat();
            if (!isAgreed && !commandFormat.isExternalUsable()) {
            ...
            for (Parameter parameter : parameters) {
                ...
                } else if (CommandFormat.class.isAssignableFrom(type)) {
                    arguments.add(commandFormat);
            ...
            if (arguments.size() == parameters.length) {
                try {
                    ...
                    // 验证权限
                    final String[] permissions = commandFormat.getPermissions();
    ...
    default Set<String> getUsageStrings(XiaomingUser user) {
        ...
        for (InteractorMethodDetail detail : getMethodDetails()) {
            if (user.hasPermission(detail.getCommandFormat().getPermissions())) {
                usages.addAll(Arrays.asList(detail.getCommandFormat().getUsages()));
            }
        }
        return usages;
    }
