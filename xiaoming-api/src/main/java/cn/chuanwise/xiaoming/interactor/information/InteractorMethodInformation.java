package cn.chuanwise.xiaoming.interactor.information;
...
public class InteractorMethodInformation {
    ...
    public InteractorMethodInformation(Method method) {
        ...
        for (Filter filter : filters) {
            if (filter.enableUsage()) {
                final String usage = StringUtility.firstNonEmpty(filter.usage(), UsageStringUtility.translateUsageRegex(filter.value()));
                if (StringUtility.nonEmpty(usage)) {
    ...
    public InteractorMethodInformation(String name, String[] formats, String[] permissions, String[] requireGroupTags, String[] requireAccountTags, boolean externalUsable, boolean quietUsable, boolean nonNext) {
        this.name = name;
        setFormats(formats);
